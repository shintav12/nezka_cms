 	...
 	Dirrty.prototype = {
 		...
 		saveInitialValues: function(){
 			...
 			this.form.find(".summernote").each(function(){
                $(this).attr("data-dirrty-initial-value", $(this).summernote('code'));
			});
 		},

        saveDinamycInitialValues: function(){
            this.form.find('#gallery_container').children().each(function () {
                $(this).find('input').each(function(){
                    if(!$(this).attr("data-dirrty-initial-value")){
                        $(this).attr("data-dirrty-initial-value", $(this).val());
                    }
                });
            });
 		...
 		setEvents: function(){
 			...
 			$(document).ready( function(){
                ...
                d.form.find(".summernote").on("summernote.change", function (e) {
                    d.checkValues();
                });
 		...
 		checkValues: function(){
 			...
 			this.form.find("input, select, textarea").each( function(){
                ...
                console.log($(this).attr('class')+": "+$(this).attr("data-dirrty-initial-value")+" -> "+$(this).val());
 				if($(this).val() != initialValue && initialValue !== undefined ){
 			...
 			this.form.find("input[type=checkbox], input[type=radio]").each( function(){
 				var initialValue = $(this).attr("data-dirrty-initial-value");
 				if($(this).is(":checked") && initialValue != "checked"  || !$(this).is(":checked")  && initialValue == "checked"){
 					$(this).attr("data-is-dirrty", "true");
				}else{
					$(this).attr("data-is-dirrty", "false");
				}
            ...
            this.form.find(".summernote").each(function () {
                var initialValue = $(this).attr("data-dirrty-initial-value");
                console.log($(this).attr('class')+": "+$(this).attr("data-dirrty-initial-value")+" -> "+$(this).summernote('code'));
                if($(this).summernote('code') != initialValue && initialValue !== undefined && initialValue !== "" && $(this).summernote('code') !== "<p></p>" && $(this).summernote('code') !== ""){
                    $(this).attr("data-is-dirrty", "true");
                }else{
                    $(this).attr("data-is-dirrty", "false");
                }
 			...
 			var isDirty = false;
 			this.form.find("input, select, textarea, .summernote").each( function(){
